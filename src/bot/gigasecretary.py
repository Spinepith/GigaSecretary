...
def analyze_document(user_id: int, user_input: str):
    analyze_prompt = '''
        Действуй строго заданному плану и правилам:
        
        ПРАВИЛА АНАЛИЗА:
        1. Анализируй ТОЛЬКО предоставленный текст
        2. Сопоставляй ИСКЛЮЧИТЕЛЬНО текст предложенного для анализа документа и ключевые слова из текста с отделами 
         и типами документов
        3. Не придумывай ничего нового. Используй строго только указанные списки


        АЛГОРИТМ АНАЛИЗА:
        1. Проанализируй предоставленный текст и ничего нового не придумываем
        2. Определи, является ли текст документом для обработки в РСЭД и можно ли его обработать
        3. Если текст является документом — классифицируй его по отделу и типу, которые разрешены тебе

        ОЧЕНЬ ЧЕТКО СОБЛЮДАТЬ ЭТИ ТРЕБОВАНИЯ:
        СЧИТАТЬ ДОКУМЕНТОМ:
...
def compare_documents(user_id: int, user_input: str, template: str):
    compare_prompt = """
        ЗАДАЧА:
        1.  ЧЕТКО определить тип предоставленного документа и отдел, к компетенции которого он относится(ничего не додумывать).
        ...
        ВЫПОЛНЯЙ ВСЕ ДЕЙСТВИЯ СТРОГО ПО ИНСТУРКЦИИ
        Инструкция по анализу:

        ШАГ 1: Классификация документа:
        Проанализируй содержание, цель и ключевые слова в документе.
        Сопоставь его с предоставленным списком типов документов и определи наиболее самый подходящий тип(ничего не додумывать).
        Исходя из типа документа и его содержания, определи, к какому отделу он может относиться, используя предоставленный список отделов.

        ШАГ 2: Проверка на соответствие шаблону:
        Проверку необходимо провести строго по следующим критериям:
        ...
        ФОРМАТ ОТВЕТА:
        Ты должен ответить, какой это тип документа и к какому отделу он может относиться.
        Так же напиши в чем не соответствия(ничего не додумывая).
