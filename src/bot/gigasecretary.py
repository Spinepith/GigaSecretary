...
main_prompt = '''
    ...
    ВАЖНО: Соблюдай деловой стиль. Избегай субъективных оценок.
...
def analyze_document(user_id: int, user_input: str):
    analyze_prompt = '''
        ...
        4. Найди 1-2 релевантные статьи законов РФ, которые могут относиться к содержанию документа
        5. Сформулируй ответ в строгом формате
        ...
        - Указывай статьи в формате: [название закона, статья номер]

        ПРАВИЛА ФОРМИРОВАНИЯ ССЫЛОК:
        - Всегда используй формат: [Название кодекса, статья X]
        - Ссылки должны вести на конкретные статьи, а не на общие страницы
        - Проверяй актуальность статей
        - Если не можешь найти конкретную ссылку, укажи только название и номер статьи
    '''
